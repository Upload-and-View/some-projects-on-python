...
def _mnm_math_randomseed(x=None):
    if isinstance(x, (int, float)):
        random.seed(x)
    else:
        # If no number is provided, you might want to seed based on system time or other entropy
        import time
        random.seed(time.time())
def _mnm_math_round(x=None): return int(math.floor(x + 0.5)) if isinstance(x, (int, float)) else 0
def _mnm_math_sin(x=None): return math.sin(x) if isinstance(x, (int, float)) else 0
...
# Populate mnm_math_lib
math_funcs = {}
for name, func in list(locals().items()):  # Iterate over a copy
    if name.startswith("_mnm_math_"):
        math_funcs[name[10:]] = func
mnm_math_lib.update(math_funcs) # Update the dictionary after iteration
mnm_math_lib['pi'] = math.pi
mnm_math_lib['huge'] = float('inf') # Lua's HUGE_VAL

# Populate mnm_string_lib
string_funcs = {}
for name, func in list(locals().items()):  # Iterate over a copy
    if name.startswith("_mnm_string_"):
        string_funcs[name[12:]] = func
mnm_string_lib.update(string_funcs) # Update the dictionary after iteration

# --- String Library Functions ---

def _get_slice_indices(length: int, i: int, j: int) -> Tuple[Optional[int], Optional[int]]:
    """Converts Lua-style string indices to Python slice indices (exclusive end)."""
    if i is None: py_i = 0
    elif i > 0: py_i = i - 1
    else: py_i = length + i # Negative index from end

    if j is None: py_j = length
    elif j >= 0: py_j = j
    else: py_j = length + j + 1 # Negative index from end (inclusive in Lua)

    if py_i < 0: py_i = 0
    if py_j > length: py_j = length
    if py_i >= length or py_j <= 0 or py_i >= py_j: return None, None # Invalid slice
...
def _mnm_string_byte(s: str="", i: int=1, j: Optional[int]=None):
    ...
    s_len = len(s)
    start, end_exclusive = _get_slice_indices(s_len, i, j if j is not None else i)
    if start is None or end_exclusive is None: return () # Empty tuple for invalid range
    return tuple(ord(char) for char in s[start:end_exclusive])

def _mnm_string_char(*args):
    return "".join(chr(int(arg)) for arg in args if isinstance(arg, (int, float)))

def _mnm_string_find(s: str="", pattern: str="", init: int=1, plain: bool=False):
    if not isinstance(s, str) or not isinstance(pattern, str): return None
    start_index = max(0, init - 1)
    if plain:
        found_index = s.find(pattern, start_index)
        if found_index != -1:
        ...
        try:
            ...
            if match:
                return match.start() + start_index + 1, match.end() + start_index
            else:
                return None
        except re.error as e:
...
def _mnm_string_format(format_string: str, *args):
    # Basic placeholder - needs more robust implementation for Lua format specifiers
    placeholders = re.findall(r'%[cdeEfgGiouxXqs]', format_string)
    ...
    for placeholder in placeholders:
        if arg_index < len(args):
            ...
            try:
                if placeholder == '%s':
                    formatted_string = formatted_string.replace(placeholder, str(value), 1)
                elif placeholder in ['%d', '%i']:
                    formatted_string = formatted_string.replace(placeholder, str(int(value)), 1)
                elif placeholder in ['%f', '%g', '%e', '%E', '%G']:
                    formatted_string = formatted_string.replace(placeholder, str(float(value)), 1)
                elif placeholder == '%q':
                    formatted_string = formatted_string.replace(placeholder, repr(value), 1) # Use Python's repr
                elif placeholder == '%c':
                    formatted_string = formatted_string.replace(placeholder, chr(int(value)) if isinstance(value, (int, float)) else '', 1)
                elif placeholder in ['%o', '%u', '%x', '%X']:
                    formatted_string = formatted_string.replace(placeholder, format(int(value), placeholder[1:]), 1)
                else:
                    formatted_string = formatted_string.replace(placeholder, str(value), 1) # Default
            except ValueError:
                print(f"Warning: Invalid value for format specifier '{placeholder}': {value}")
                formatted_string = formatted_string.replace(placeholder, '<error>', 1)
            arg_index += 1
        else:
            formatted_string = formatted_string.replace(placeholder, '<missing>', 1)
    return formatted_string

def _mnm_string_gmatch(s: str="", pattern: str=""):
    ...
    try:
        ...
        print(f"Regex error in string.gmatch pattern '{pattern}': {e}")
        return iter([])

def _mnm_string_gsub(text: str, pattern: str, replacement, n: Optional[int] = None):
    return lua_gsub(text, pattern, replacement) if n is None else lua_gsub(text, pattern, replacement)[:n]
...
def _mnm_string_split(s: str="", separator: Optional[str] = None):
    if not isinstance(s, str): s = str(s)
    if separator is None:
        return [s] # Default behavior if no separator provided
...
def MNM2(code, local_vars=None, is_block_execution=False):
    ...
    multiline_comment_state = None # None, '--[[', '--[='
    ...
    while current_line_index < len(lines):
        ...
        if multiline_comment_state:
            if multiline_comment_state == '--[[' and line.find(']]') != -1:
                multiline_comment_state = None
            elif multiline_comment_state == '--[=' and line.find(']=]') != -1:
                multiline_comment_state = None
                 ...
                 if len(rhs_tokens) > 0:
                     ...
                     if callable(potential_func):
                         ...
                         if isinstance(call_result, tuple):
                             local_vars[variable_name] = call_result[0] # Assign the first value
                         else:
