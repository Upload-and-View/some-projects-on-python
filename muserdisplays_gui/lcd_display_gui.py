# /home/muser/Desktop/proeject/muserdisplays_gui/lcd_display_gui.py
...
class LCD16x2GUI(GUIDisplay):
    ...
    WIDTH_PX = NUM_COLS * CHAR_WIDTH_PX
    HEIGHT_PX = NUM_ROWS * CHAR_HEIGHT_PX

    ACTIVE_COLOR = "white"
    INACTIVE_COLOR = "darkblue"
    ...
    def __init__(self, canvas: tk.Canvas, start_x: int = 0, start_y: int = 0):
        ...
        self.pixels = [] 
        self.PIXEL_SIZE = 2 # Added scaling factor for LCD
    ...
    def write_text(self, row: int, col: int, text: str):
        ...
        for char in text:
            self.draw_char_to_buffer(current_x_px, row * self.CHAR_HEIGHT_PX, char)
            current_x_px += self.CHAR_WIDTH_PX 

    def render_empty_display(self):
        # Clear existing pixel drawing items from previous renders (if any)
